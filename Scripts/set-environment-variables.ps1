...
# Generate Frontend .env file from the same environment variables
# The Frontend Express server uses dotenv to load config from this file.
# Frontend/.env is gitignored (root .gitignore and Frontend/.gitignore) so secrets are never committed.
Write-Host ""
Write-Host "🔄 Generating Frontend .env file..." -ForegroundColor Cyan
...
if (Test-Path $frontendDir) {
    try {
        $frontendEnvContent = @(
            "# Auto-generated by azd post-provision hook (Scripts/set-environment-variables.ps1)"
            "# This file is gitignored — do not check in. Re-run 'azd provision' to regenerate."
            ""
            "# APIM Gateway URL with API path — exposed to browser (VITE_ prefix)"
            "VITE_APIM_GATEWAY_URL=$($envVars['apim_gateway_url'])/unified-ai"
            ""
            "# APIM Subscription Key — server-side only"
            "APIM_SUBSCRIPTION_KEY=$($envVars['apim_subscription_key'])"
            ""
            "# APIM Resource ID for management API — server-side only"
            "APIM_RESOURCE_ID=$($envVars['apim_resource_id'])"
            ""
            "# Azure AD / Entra ID credentials — server-side only"
            "TENANT_ID=$($envVars['tenant_id'])"
            "ENTRA_APP_ID=$($envVars['entra_app_id'])"
            "ENTRA_APP_CLIENT_SECRET=$($envVars['entra_app_client_secret'])"
        ) -join "`n"

        Set-Content -Path $frontendEnvPath -Value $frontendEnvContent -Encoding UTF8 -ErrorAction Stop
        Write-Host "✅ Frontend .env file generated successfully" -ForegroundColor Green
        Write-Host "   📂 Location: $frontendEnvPath" -ForegroundColor Gray
        Write-Host "   🔒 This file is gitignored and contains secrets — do not commit" -ForegroundColor Gray
    } catch {
        Write-Warning "⚠️  Could not generate Frontend .env file: $($_.Exception.Message). Create Frontend/.env manually using Frontend/.env.example as a template"
    }
} else {
    Write-Host "⏭️  Frontend directory not found — skipping .env generation" -ForegroundColor Yellow
}
