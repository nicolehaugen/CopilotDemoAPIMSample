...
applyTo: '**/*.terraform, **/*.tf, **/*.tfvars, **/*.tflint.hcl, **/*.tfstate, **/*.tf.json, **/*.tfvars.json, **/*.xml'
---
...
These instructions provide Azure-specific guidance for solutions created Terraform.
...
Follow coding standards in solution code to maintain consistency:
...
**CRITICAL - File and Folder Casing**: Never change file or folder name casing without explicit user approval. Path references are case-sensitive on Linux but not on Windows. Changing casing can break deployments when code moves between platforms (e.g., policy fragment paths like `Infra/Resources/Fragments/` must match exactly in all references).

For production solutions with multiple environments, use tfvars to modify environmental differences. For sample repositories, a single tfvars file is acceptable.
...
│   ├── main.tfvars.json            # Variable values (or environments/ folder for multi-env)
│   ├── Modules/                    # Reusable Terraform modules
│   └── Resources/                  # Non-Terraform resources deployed via Terraform
│       ├── Policies/               # APIM policy XML files
│       │   ├── APIPolicies/        # Main API policies
│       │   │   └── *.xml
│       │   └── ProductPolicies/    # Product-level policies (JWT, subscription)
│       │       └── *.xml
│       ├── Fragments/              # APIM policy fragment XML files
│       │   └── *.xml
│       └── Schema/                 # API schema files (JSON, etc.)
├── .github/workflows/              # CI/CD pipelines (if using github)
└── README.md                       # Documentation
```

Never change the folder structure or file/folder casing without direct agreement with the user.
