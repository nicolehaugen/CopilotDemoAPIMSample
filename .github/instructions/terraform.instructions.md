...
- **Preserve file and folder casing**: Never change file or folder name casing without explicit user approval. Path references are case-sensitive on Linux but not on Windows, so changing casing can break deployments (e.g., policy fragment paths, module source paths). Always verify path references match exactly after any file operations.
...
- Store sensitive information in a secure manner, such as using Azure Key Vault.
  - Regularly rotate credentials and secrets.
  - Automate the rotation of secrets, where possible.
- Use environment variables to reference values stored in secure vaults.
...
- Use role-based access control (RBAC) to control access to resources.
  - Follow the principle of least privilege when assigning permissions.
- Use network security groups and firewalls to control network access to resources.
- Deploy resources in private networks whenever possible.
  - Use public endpoints only for resources that require direct internet access.
- Use encryption for sensitive data at rest and in transit.
  - Enable encryption for storage accounts, databases, and other data stores.
...
- Include a `README.md` file at the project root to provide an overview and setup instructions.
...
- Ensure configurations are idempotent (multiple applies produce the same result).
- For sample repositories, rely on validation rules in variable blocks to enforce constraints.
